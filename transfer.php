...
if (!isset($b['balance']) || $b['balance'] < $amount) {
    echo "Insufficient b balance.";
    exit;
}
...
$ownerUsername = $b['owner'] ?? '';
$ownerFound = false;

foreach ($accounts as &$acc) {
    if ($acc['username'] === $ownerUsername) {
        $b['balance'] -= $amount;
        $acc['wallet_balance'] += $amount;
        $ownerFound = true;
        break;
    }
}
